...
| **One claim, on the entity** | Leaving the world is `SimEntity.claim_snapshot()` — one flag, one route, shared by every affordance that takes a thing. `PickUpAbleComponent` and `ConsumableComponent` are **doors**, not mechanisms: each adds only its own verb and signal. Previously they held independent flags (`_taken`, `_used`), so one berry could be won twice in a frame. Callers must do all their failing first, since a claim cannot be undone — and because every reason to refuse is now an actor-side fact, that ordering is checked where the knowledge already is rather than managed inside the target. |
| **Two entry points, one implementation** | `SimHungerComponent.eat_snapshot(def)` is the only place nourishment is ever applied; `eat_entity(target)` is a thin adapter that wins a world entity and delegates to it. Named separately because GDScript has no overloading and because the caller always knows which it means — a type switch inside one `eat(target)` was throwing that knowledge away and reconstructing it. The danger was never two entry points, it was two *implementations*: an adapter with no logic of its own cannot drift. `eat_entity` mirrors `SimInventoryComponent.try_pick_up()`, so both affordances are encapsulated on the actor's component instead of one leaking into the brain. Verified identical at +35.0 each. |
| **Nobody disposes of someone else's item** | `SimHungerComponent.eat_snapshot()` only restores itself, then announces `SimEntity.thing_used(verb, target)`; whatever holds that snapshot drops it. Hunger never references Inventory, Inventory never references Hunger — the entity mediates. |
...
| **Each side resolves only what it alone can know** | The actor checks its own facts — reach, capacity — and the target checks its own — availability. Neither computes the other's half, and the message between them describes an attempt, never an outcome. `InventoryComponent.try_pick_up()` asks `ActionComponent` "can I reach?", asks itself "have I room?", and only then asks the target's `PickUpAbleComponent` to hand itself over. The target has no opinion on pockets and cannot see them. This is the shape combat will need: an attacker resolves its own weapon and crit, the target resolves its own armour and dodge, so adding a defence never touches the attacker. |
| **Actor asks, target resolves** | Affordances never reach into the actor. Action is *the hand* — it knows no specific action, so a future `AttackComponent` reuses it. Movement is *the legs*. An affordance is only ever **a verb plus a claim**: `PickUpAbleComponent.claim()` and `SimConsumableComponent.claim()` are the same four lines with a different verb and signal. |
