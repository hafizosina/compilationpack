class_name SimBrainFSMComponent
extends SimBrainComponent

## A hand-rolled state machine brain — the simple AI, and one of possibly
## several ways to fill the `brain` slot. A GOAP planner will be another; both
## occupy the same slot, so nothing that talks to "the brain" has to change.
##
## What an entity can do is decided by which OTHER components it carries, not by
## which brain it has: this same class drives a creature with no inventory or no
## action component, it just does less with it.
##
...
func _ready() -> void:
	...
	if _sensor == null or _movement == null:
		push_warning("SimBrainFSMComponent on '%s' needs a sensor and movement component" % entity.name)
		set_process(false)
		return
	_rng.randomize()
...
func _feed_if_hungry() -> bool:
	...
	if _inventory != null:
		...
		if carried != null:
			# Nothing to walk to — it is already in hand, and already won.
			_hunger.eat_snapshot(carried)
			_target = null
			_enter(State.WANDER)
			return true
	...
	if _action != null and _action.in_reach(found):
		# Eaten where it lies; it never enters the inventory. Hunger wins it out
		# of the world itself, the same way Inventory wins a pick-up.
		_hunger.eat_entity(found)
		_target = null
		_enter(State.WANDER)
		return true
